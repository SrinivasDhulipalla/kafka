...
public class OffsetForLeaderEpochRequest extends AbstractRequest {
    ...
    public AbstractResponse getErrorResponse(Throwable e) {
        ...
        Map<String, List<EpochEndOffset>> errorResponse = new HashMap();
        for (String topic : epochsByTopic.keySet()) {
            List<EpochEndOffset> resultEpoch = new ArrayList();
            for (Epoch epoch : epochsByTopic.get(topic)) {
                resultEpoch.add(new EpochEndOffset(error, epoch.partitionId()));
            }
            errorResponse.put(topic, resultEpoch);
        }

        return new OffsetForLeaderEpochResponse(errorResponse);
    }
}