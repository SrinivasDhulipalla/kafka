...
public class OffsetForLeaderEpochResponse extends AbstractResponse {
    private static final Schema CURRENT_SCHEMA = ProtoUtils.currentResponseSchema(ApiKeys.OFFSET_FOR_LEADER_EPOCH.id);
    ...
    public static OffsetForLeaderEpochResponse parse(ByteBuffer buffer, int versionId) {
        return new OffsetForLeaderEpochResponse(ProtoUtils.parseResponse(ApiKeys.OFFSET_FOR_LEADER_EPOCH.id, versionId, buffer));
    }

    public static OffsetForLeaderEpochResponse parse(ByteBuffer buffer) {
        return new OffsetForLeaderEpochResponse(CURRENT_SCHEMA.read(buffer));
    }
}
