...
object KafkaConfig {
  ...
  private val configDef = {
    ...
    new ConfigDef()
      ...
      .define(SSLCipherSuitesProp, LIST, Defaults.SSLCypherSuites, MEDIUM, SSLCipherSuitesDoc)
...
case class KafkaConfig (props: java.util.Map[_, _]) extends AbstractConfig(KafkaConfig.configDef, props) {
  ...
  val sslCypher = getList(KafkaConfig.SSLCipherSuitesProp)
  ...
  def channelConfigs: java.util.Map[String, Object] = {
    ...
    channelConfigs.put(SSLCipherSuitesProp, sslCypher)
    channelConfigs
  }
