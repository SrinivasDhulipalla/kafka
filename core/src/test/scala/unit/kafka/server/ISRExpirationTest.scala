...
class IsrExpirationTest {
  ...
  @Before
  def setUp() {
    replicaManager = new ReplicaManager(configs.head, metrics, time, jTime, null, null, null, new AtomicBoolean(false), QuotaManagerFactory.instantiate(configs.head, metrics))
  }
