...
class ReplicaManagerTest {
  ...
  def testHighWaterMarkDirectoryMapping() {
    ...
    val rm = new ReplicaManager(config, metrics, time, jTime, zkUtils, new MockScheduler(time), mockLogMgr,
      new AtomicBoolean(false), QuotaManagerFactory.instantiate(config, metrics))
    try {
  ...
  def testIllegalRequiredAcks() {
    ...
    val mockLogMgr = TestUtils.createLogManager(config.logDirs.map(new File(_)).toArray)
    val rm = new ReplicaManager(config, metrics, time, jTime, zkUtils, new MockScheduler(time), mockLogMgr,
      new AtomicBoolean(false), QuotaManagerFactory.instantiate(config, metrics), Option(this.getClass.getName))
    try {
